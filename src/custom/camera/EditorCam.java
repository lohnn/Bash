 ...
 * @author lohnn
 */
public class EditorCam implements Control, AnalogListener, ActionListener
{
    ...
    public EditorCam(Camera cam, InputManager inputManager, Spatial target, Node rootNode)
    {
        this.cam = cam;
        camNode = new Node();
...
//        registerInput(inputManager);
    ...
    public void onAnalog(String name, float value, float tpf)
    {
        ...
        setLocation(camNode.getLocalTranslation().addLocal(direction.multLocal(moveSpeed * tpf * chaseCam.getDistanceToTarget() / 20)));

        //This was a try to get dynamic height on camera depending on how the scene looks
        //TODO: fix this
//        Ray ray = new Ray(camNode.getLocalTranslation().addLocal(0, 100, 0), camNode.getLocalTranslation().subtractLocal(0, 100, 0));
//        CollisionResults results = new CollisionResults();
//        rootNode.getChild("SceneNode").collideWith(ray, results);
//        if(results.size() > 0) {
//            CollisionResult closest = results.getClosestCollision();
//            setLocation(camNode.getLocalTranslation().setY(closest.getContactPoint().y));
//        }else {
//        }
    }
    ...
    {
        EditorCam other = new EditorCam(cam, inputManager, spatial, rootNode);
        return other;
    }
